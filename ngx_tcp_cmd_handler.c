...
ngx_tcp_cmd_process_init(ngx_cycle_t *cycle)
{
	...
	load_ctx = ngx_pcalloc(cycle->pool, sizeof(ngx_tcp_global_ctx_t));
	if (NULL == load_ctx) {
		goto failed;
	}
	
	load_ctx->conf_get_str = (ngx_tcp_conf_get_str_pt)ngx_tcp_cmd_conf_get_str;
    ...
    for (i=0; i < cmdso_mgr->cmdsos.nelts; ++i) {
        if (cmdsos[i].cmdso_load(cycle, ngx_tcp_cmd_pkg_handler_add, i, load_ctx)
            != NGX_OK) {
            goto failed;
        }
    }
