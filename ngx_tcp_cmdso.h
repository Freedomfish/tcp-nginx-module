...
typedef void (*ngx_cyl_log_error_pt)(ngx_tcp_uint_t level, void *log, 
                                     ngx_tcp_err_t err, 
                                     const char *fmt, ...);
struct ngx_tcp_global_ctx_s {
	   ngx_tcp_conf_get_str_pt  conf_get_str;
	   void                     *log;
	   ngx_cyl_log_error_pt     log_error;
};
typedef struct ngx_tcp_global_ctx_s ngx_tcp_global_ctx_t;
typedef struct ngx_tcp_global_ctx_s ngx_tcp_load_ctx_t;

typedef long 
(*cmdso_load_pt)(void *cycle_param, cmd_pkg_handler_add_pt add_h, int slot, ngx_tcp_load_ctx_t *load_ctx);
